...
phelp_mainMapRead(
   ...
   *     - output file
   ...
   fprintf(
      (FILE *) outFILE,
      "    -ref-index reference.index: [Replaces -ref]\n"
   );

   fprintf(
      (FILE *) outFILE,
      "      o reference file output by -index\n"
   );
   ...
   fprintf(
      (FILE *) outFILE,
      "    -index reference.fasta: [Disabled]\n"
   );

   fprintf(
      (FILE *) outFILE,
      "      o print reference.fasta index to -out\n"
   );

   fprintf(
      (FILE *) outFILE,
      "      o no alignment is done, so -fq is ignored\n"
   );
   ...
   *   - variable chaining variables (length,size,%)
   ...
   fprintf(
      (FILE *) outFILE,
      "  kmer/chaining variables:\n"
   );

   fprintf(
      (FILE *) outFILE,
      "    -kmer-val length,size,chain: [Optional]\n"
   );
   ...
   *     - min percent length
   ...
   if(def_alnEnd_defsMapRead)
      fprintf(
         (FILE *) outFILE,
         "    -aln-ends: [Optional; Yes]\n"
      );
   else
      fprintf(
         (FILE *) outFILE,
         "    -aln-ends: [Optional; No]\n"
      );

   fprintf(
      (FILE *) outFILE,
      "      o align 1.5 * (min bases in chain) soft\n"
   );

   fprintf(
      (FILE *) outFILE,
      "        masked bases at alignment ends\n"
   );

   fprintf(
      (FILE *) outFILE,
      "      o disable with \"-no-aln-ends\"\n"
   );
   ...
   fprintf(
      (FILE *) outFILE,
      "    -max-perc-len %0.2f: [Optional; %0.2f]\n",
   ...
   fprintf(
      (FILE *) outFILE,
      "      o maximun gap (as percent query lenth) allowed\n"
   );

   fprintf(
      (FILE *) outFILE,
      "        between chains (for chain merging)\n"
   );
   ...
   fprintf(
      (FILE *) outFILE,
      "    -min-perc-score %0.2f: [Optional; %0.2f]\n",
   ...
   fprintf(
      (FILE *) outFILE,
      "    -min-perc-match %0.2f: [Optional; %0.2f]\n",
   ...
   fprintf(
      (FILE *) outFILE,
      "      o mininum percent matches to keep aligment\n"
   );
      ...
      *   - kmer-val query length, kmer size, and % kmer
      \**************************************************/
         ...
         if(
            ...
            fprintf(
               stderr,
               "-kmer-val %s; kmer size > 15; to large\n",
               argAryStr[siArg]
            );
      ...
      *     - min percent length
      ...
      \+++++++++++++++++++++++++++++++++++++++++++++++++*/
         ...
         { /*If: error*/
            fprintf(
               stderr,
               "-max-per-len %s; non-numeric/to large\n",
               argAryStr[siArg]
            );
         ...
         { /*If: out of bounds*/
            fprintf(
               stderr,
               "-max-per-len %s must be positive\n",
               argAryStr[siArg]
            );
      ...
      \+++++++++++++++++++++++++++++++++++++++++++++++++*/
         ...
         { /*If: error*/
            fprintf(
               stderr,
               "-min-perc-score %s; non-num/to large\n",
               argAryStr[siArg]
            );
         ...
         { /*If: overflow*/
            fprintf(
               stderr,
               "-min-perc-score %s; must be 1 or less\n",
               argAryStr[siArg]
            );
         ...
         { /*If: error*/
            fprintf(
               stderr,
               "-min-perc-match %s non-numeric/toLarge\n",
               argAryStr[siArg]
            );
...
main(
   ...
   signed int numAlnSI = 0;
   signed int sizeSamSI = 0;
   ...
   */
   ...
   { /*Else: getting reference from index file*/
      ...
      { /*If: had memory error*/
         fprintf(
            stderr,
            "memory error reading -ref-index %s\n",
            refFileStr
         );
      ...
      { /*If: had memory error*/
         fprintf(
            stderr,
            "file error reading -ref-index %s\n",
            refFileStr
         );
   ...
   { /*Loop: map reads*/
      ...
      scoreSL =
         align_mapRead(
            ...
            &alnStackST, /*arrays for alignment*/
            &setStackST, /*settings*/
            &samHeapAryST,/*gets alignments*/
            &numAlnSI,    /*number alignments*/
            &sizeSamSI,   /*size of alignments*/
            0,            /*no best score*/
      ...
      { /*Else If: mapped read*/ 
         for(
            ...
            scoreSL < numAlnSI;
            ++scoreSL
         ){ /*Loop: print alignments*/
            errSC =
               p_samEntry(
                  &samHeapAryST[scoreSL],
                  &buffHeapStr,
      ...
      printf(
         "%li\t%f\n",
         seqSL,
         (double) (postTimeT - preTimeT) / CLOCKS_PER_SEC
      );*/
   ...
   *   - handel errors before cleanup
   ...
   ret_main_sec04:;
      ...
      for(
         ...
         scoreSL < sizeSamSI;
         ++scoreSL
      ) freeStack_samEntry(&samHeapAryST[scoreSL]);
      free(samHeapAryST);
      samHeapAryST = 0;
