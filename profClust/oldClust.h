...
'   o fun14: boom_profile_samClust
'     - marks (boom) reads that were discarded (did not
'       map to profile)
'   o fun15: prob_clustSam
...
'   o fun16: roughProb_clustSam
'     - finds a rough estiamte (slighty off) of the
'       expected power of 10 from prob_clustSam
'   o fun17: pickProfile_clustSam
'     - picks the "best" profile from the history of a
'       profile_clustSam struct
'   o fun18: bin_clustSam
'     - bin picked cluster; call after fun17
\~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
   ...
   if(
      ...
      if(
         ...
         for(
            ...
            if(profSTPtr->varArySI)
               profSTPtr->varArySI[uiCnt] = -1;
...
addReadTo_profile_clustSam(
   ...
   ) goto discardedRead_fun12_sec04_sub03;
   ...
   { /*Loop: move to first profile position*/
      ...
      if(profPosUI >= profSTPtr->lenProfileUI)
         goto done_fun12_sec05_sub01; /*nothing in profile*/
   } /*Loop: move to first profile position*/
         ...
         /*Case: snp*/
            ...
            { /*Loop: find number of good SNPs*/
               ...
               while(
                  ...
                  if(profPosUI > profSTPtr->lenProfileUI)
                     goto done_fun12_sec05_sub01; /*done*/
               } /*Loop: move past insertions*/
   ...
   *   - no error clean up
   ...
   done_fun12_sec05_sub01:;
   errSC = 0;
   goto ret_fun12_sec05_sub05;
   ...
   *   - map error clean up
   ...
   /*****************************************************\
   * Fun12 Sec05 Sub03:
   *   - discarded read error clean up
   \*****************************************************/

   discardedRead_fun12_sec04_sub03:;
   ...
   *   - memory error clean up
   ...
   memErr_fun12_sec05_sub04:;
   errSC = def_memErr_smaClust;
   goto ret_fun12_sec05_sub05;

   /*****************************************************\
   * Fun12 Sec05 Sub05:
   *   - return + general clean up (everything calls)
   \*****************************************************/

   ret_fun12_sec05_sub05:;
...
|         appeared
|       o varArySI to have varaint index's (profile)
...
boom_profile_samClust(
   ...
   slong depthSL = 0; /*number of reads kept*/
   ...
   { /*If: error*/
      if(errSC == def_memErr_samEntry)
         ...
         goto fileErr_fun14_sec05_sub03;
   } /*If: error*/
   ...
   *   - check matchs + start loops
   ...
   while(errSC)
   { /*Loop: get reads by profile*/
      if(profSTPtr->readHitArySI[profPosUI] < uiHist - 1)
         goto nextEntry_fun14_sec04_sub06;
         /*read was discarded, not in profile*/
      ...
      for(
         ...
         { /*Switch: profile cigar*/
            ...
            /*Case: match to reference*/
               ...
               while(
                 refPosUI > profSTPtr->varArySI[profPosUI]
               ){ /*Loop: move to next varaint*/
                  ++profPosUI;

                  if(profPosUI >= profSTPtr->lenProfileUI)
                     goto checkProfile_fun14_sec04_sub05;
               } /*Loop: move to next varaint*/
            ...
            *     -  move to snp varaint position
            *   o fun14 sec04 sub02 cat02:
            *     -  check if keep SNP; in profile
            *   o fun14 sec04 sub02 cat03:
            *     - finsihed snp; move to next variant
            \********************************************/
            ...
            /*Case: SNP to reference*/
               ...
               { /*Loop: find snps in profile*/
                  ...
                  while(
                     ...
                     if(
                       profPosUI > profSTPtr->lenProfileUI
                     ) goto done_fun14_sec05_sub01;
                  } /*Loop: move past insertions*/
                  ...
                  errSC =
                     ...
                     ); /*see if have match to profile*/

                  if(
                      refPosUI
                    < profSTPtr->varArySI[profPosUI]
                  ) ; /*snp is not in profile*/

                  else if(errSC)
                     ++uiVar; /*one more variant*/
            ...
            * Fun14 Sec04 Sub03:
            *   - check deletions
            *   o fun14 sec04 sub03 cat01:
            *     - check if keep deletion
            *   o fun14 sec04 sub03 cat02:
            *     - move to closest base in profile
            *   o fun14 sec04 sub03 cat03:
            *     - check if keep deletion;(in profile
            *   o fun14 sec04 sub03 cat04:
            *     - prepare for next variant
            \********************************************/

            /*+++++++++++++++++++++++++++++++++++++++++++\
            + Fun14 Sec04 Sub03 Cat01:
            +   - check if keep deletion
            \+++++++++++++++++++++++++++++++++++++++++++*/

            case 'D':
            /*Case: deletion to reference*/
               tmpSI = samSTPtr->cigTypeStr[cigPosUI];

               if(tmpSI < (sint) minIndelLenUI)
                  goto skipDel_fun14_sec04_sub03;
               ...
               /*++++++++++++++++++++++++++++++++++++++++\
               + Fun14 Sec04 Sub03 Cat02:
               +   - move to closest base in profile
               \++++++++++++++++++++++++++++++++++++++++*/

               while(tmpSI > 0)
               { /*Loop: count matching deletions*/
                  while(
                       refPosUI
                    > profSTPtr->varArySI[profPosUI]
                  ){ /*Loop: move to next varaint*/
            ...
            * Fun14 Sec04 Sub04:
            *   - check insertions
            *   o fun14 sec04 sub04 cat01:
            *     - check if have insertion to keep
            *   o fun14 sec04 sub04 cat02:
            *     - single insertion case
            *   o fun14 sec04 sub04 cat03:
            *     - align insertion (>1 insertion)
            *   o fun14 sec04 sub04 cat04:
            *     - check alignment cigar for matches
            *   o fun14 sec04 sub04 cat0x:
            *     - finish up insertion entry
            \********************************************/

            /*+++++++++++++++++++++++++++++++++++++++++++\
            + Fun14 Sec04 Sub04 Cat01:
            +   - check if have insertion to keep
            \+++++++++++++++++++++++++++++++++++++++++++*/

            case 'I':
            /*Case: insertion to reference*/
               tmpSI = samSTPtr->cigTypeStr[cigPosUI];

               if(tmpSI < (sint) minIndelLenUI)
                  goto skipIns_fu14_sec04_sub04_cat05;
               ...
               if(profSTPtr->varArySI[profPosUI] < 0)
                  goto skipIns_fu14_sec04_sub04_cat05;
                  /*profile does not support a variant*/
               ...
               if(
                  ...
                  ++profPosUI;
                  tmpSI = 0;
                  goto skipIns_fun14_sec04_sub04_cat05;
               } /*If: insertion is one base long*/

               /*++++++++++++++++++++++++++++++++++++++++\
               + Fun14 Sec04 Sub04 Cat03:
               +   - align insertion (>1 insertion)
               \++++++++++++++++++++++++++++++++++++++++*/
               ...
               /*++++++++++++++++++++++++++++++++++++++++\
               + Fun14 Sec04 Sub04 Cat05:
               +   - finish up insertion entry
               \++++++++++++++++++++++++++++++++++++++++*/

               skipIns_fun14_sec04_sub04_cat05:;
               ...
               { /*Loop: move past unused insertions*/
                  ...
                  if(profPosUI > profSTPtr->lenProfileUI)
                     goto done_fun14_sec05_sub01; /*done*/
               } /*Loop: move past unused insertions*/
      ...
      *   - check if keep read
      ...
      checkProfile_fun14_sec04_sub05:;

      if(uiVar >= profSTPtr->cntAryUI[uiHist])
      { /*If: read matches profile*/
         profSTPtr->readHitArySI[readUL] = uiHist;
         ++depthSL;
      } /*If: read matches profile*/
   ...
   ^     - no error clean up
   ^   o fun14 sec05 sub01:
   ^     - memroy error clean up
   ^   o fun14 sec05 sub01:
   ^     - file error clean up
   ^   o fun14 sec05 sub04:
   ^     - general clean up and return (everything calls)
   \<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
   ...
   *   - no error clean up
   ...
   done_fun14_sec05_sub01:;
   profSTPtr->depthAryUI[uiHist] = depthSL;
   goto ret_fun14_sec05_sub04;
   ...
   *   - memory error clean up
   ...
   memErr_fun14_sec05_sub02:;
   depthSL = def_memErr_clustSam;
   goto ret_fun14_sec05_sub04;
   ...
   *   - file error clean up
   ...
   fileErr_fun14_sec05_sub03:;
   depthSL = def_fileErr_clustSam;
   goto ret_fun14_sec05_sub04;
...
/*-------------------------------------------------------\
| Fun16: roughProb_clustSam
|   - finds a rough estiamte (slighty off) of the expected
|     power of 10 from prob_clustSam
|   - always use prob_clustSam to verify
| Input:
|   - numVarUI:
|     o number of variants in profile
|   - lenTargUI:
...
roughProb_clustSam(
   ...
   return
      (slong)
        ( (depthDigUI * depthUI) + (lenDigUI * numVarUI) )
      - (errDigUI * numVarUI * depthUI);
...
/*-------------------------------------------------------\
| Fun17: pickProfile_clustSam
|   - picks the "best" profile from the history of a
|     profile_clustSam struct
| Input:
|   - profSTPtr:
|     o pointer to profile_clustSam struct with history
|       to pick profile from
|   - totoalReadsUI:
|     o number of reads in sam file that mapped to the bin
|   - errF:
|     o expected error rate (0 to 1)
|   - minDepthUI:
|     o minimum read depth to keep profile
|   - minProbF:
|     o minimum probability to keep profile (0 to 1)
| Output:
|   - Modifies:
|     o readHistArySI in profSTPtr to have kept reads
|       as 1, and discarded reads as
|       def_profileDiscard_clustSam
|   - Returns:
|     o probability of best profile
\-------------------------------------------------------*/
double
pickProfile_clustSam(
   struct profile_clustSam *profSTPtr,
   float errF,
   unsigned int minDepthUI,
   float minProbF
   ...
   '     - variable declarations
   '   o fun17 sec02:
   '     - find best probability in history (rough method)
   '   o fun17 sec03:
   '     - check if best probability supports new cluster
   '   o fun17 sec04:
   '     - mark reads as kept or discard in cluster
   \~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
   ...
   for(
      ...
      uiHist < histIndexUI;
      ++uiHist
   ){ /*Loop: find best probablity (withing power 10)*/
      pow10SL =
         roughProb_clustSam(
            profSTPtr->cntAryUI[uiHist],
            profSTPtr->lenProfileUI,
            profSTPtr->depthAryUI[uiHist],
            profSTPtr->lenHitUL,
            errF
         );

      if(pow10SL < 0)
      { /*If: expect faction back*/
         if(bestPowSL > pow10SL)
         { /*If: new hit is better*/
            bestPowSL = pow10SL;
            profSTPtr->bestHistUI = uiHist;
         } /*If: new hit is better*/
      } /*If: expect faction back*/
   } /*Loop: find best probablity (within power 10)*/
   ...
   if(power10SL >= 0)
      goto lowProb_fun17_sec05_sub02;
      /*probability supports error*/

   uiHist = profSTPtr->bestHistUI;
   ...
   for(
      ulRead = 0;
      ulRead < profSTPtr->lenHitUL;
      ++ulRead
   ){ /*Loop: mark reads keeping*/
      if(profSTPtr->readHitArySI[ulRead] >= uiHist)
         profSTPtr->readHitArySI[ulRead] = 1;

      else if(profSTPtr->readHitArySI[ulRead] > 0)
         profSTPtr->readHitArySI[ulRead] = 
            def_profileDiscard_clustSam;
   } /*Loop: mark reads keeping*/
   ...
   ^     - no error clean up
   ^   o fun17 sec05 sub02:
   ^     - to high error probility clean up
   ^   o fun17 sec05 sub03:
   ^     - return probability
   \<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
